...
async function chainPromises(array, action) {
  const result = [];
  const res = await array.reduce((promiseChain, curTask) => promiseChain.then(() => {
    curTask.then((curRes) => result.push(curRes)).catch(() => {});
  }), Promise.resolve([])).then(() => result.reduce(action));
  return res;
}
